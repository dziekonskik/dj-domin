...
export function getPrevSongIndex(state: State) {
  return state.activeIndex - 1 > 0 ? state.activeIndex - 1 : state.tracks.length - 1;
}

export function getNextSongIndex(state: State) {
  return state.activeIndex + 1 < state.tracks.length ? state.activeIndex + 1 : 0;
}

export function fromPrevPageIndex(state: State) {
  const currentPageIndex = Math.floor(state.activeIndex / TRACKS_PER_PAGE);
